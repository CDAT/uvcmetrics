...
from acme_diags.metrics.defines import Hemispheres
...
def plot(polar_data):
    ...
    for key, template_id, isofill, title in zip(keys, template_ids, isofills, titles):
        ...
        data = polar_data[key](latitude=Hemispheres[hemisphere], longitude=(-10,370))
        data.id = names[key]
        nlevels = 16
...
cloud_data = {}
cloud_data['model'], cloud_data['obs'] = regrid_to_common_grid(model, obs, regridMethod=regridMethod, regridTool=regridTool)
