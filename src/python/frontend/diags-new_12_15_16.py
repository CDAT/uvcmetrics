...
import hashlib, os, pickle, sys, os, time, re, pdb, logging, itertools
from metrics import *
...
def process_table(opts, modelfts, obsfts, sclass, outdir):
    # make tables
    varid = None
    ...
    table = sclass(modelfts, obsfts, varid=varid, obsfilter=obsfilter, dryrun=opts['dryrun'], sbatch=opts["sbatch"],
                   computeall=computeall, outdir=outdir)
...
def init(opts):
    """This is the init method that takes the options from the command line and does a basic level
    of setup before generating the plots or table. The intent is to get all of the prossesing of opt
    here and pass the results for execution. In the future this will be the init method of a class."""

    def get_variables(sname, modelfts, obsfts, pclass, opts):
    ...
    #recover nicknames of model and obs for display purposes
    ...
    plot_menu = pclass.list_diagnostic_sets() #formerly sm

    if opts['sets'] is None:
        keys = plot_menu.keys()
        keys.sort()
        NAME = keys[1]
        varids = get_variables(NAME, modelfts, obsfts, pclass, opts)
        all_variables = pclass.all_variables( modelfts, obsfts, NAME )
        plotset = {'name':NAME, 'plot_type':plot_menu[NAME], 'varids':varids, 'all_variables':all_variables}
        plotsets = [plotset]#[ keys[1] ]
        logger.warning("plot sets not specified, defaulting to %s",plotsets[0])
    else:
        sndic = { setnum(s):s for s in plot_menu.keys() }   # plot set number:name
        plotsets = []
        for ID in opts['sets']:
            NAME = sndic[ID]
            varids = get_variables(NAME, modelfts, obsfts, pclass, opts)
            all_variables = pclass.all_variables(modelfts, obsfts, NAME)
            plotset = {'name': sndic[ID], 'plot_type': plot_menu[sndic[ID]], 'varids': varids, 'all_variables':all_variables }
            plotsets += [plotset] #[sndic[ID]]
    ...
    times = list(set(times) & set(pclass.list_seasons()))

    return package, plotsets, modelfts, obsfts, outdir, basename, postname, vcanvas, vcanvas2, times, regions, names

def run_diags( opts ):
    global vcs_elements

    package, plotsets, modelfts, obsfts, outdir, basename, postname, vcanvas, vcanvas2, times, regions, names \
        = init( opts )

    # Ok, start the main loops.
    number_diagnostic_plots = 0

    INPUTS = itertools.product(plotsets, times, regions)
    for plotset, time, region in INPUTS:
        ps_name = plotset['name']
        logger.info("Working on %s plots ",  ps_name)

        # retrieve the plot class
        sclass = plotset['plot_type']
        ...
        if (sclass.number == '1' and package.upper() == 'AMWG'):
            process_table(opts, modelfts, obsfts, sclass, outdir)
            continue
        ...
        # loop over varids now
        varids = plotset['varids']
        vcount = len(varids)
        counter = 0
        for ivarid, varid in enumerate(varids):
            logger.info("Processing variable %s in season %s in plotset %s -variable %s of %s", varid, time, ps_name, counter, vcount)
            counter = counter+1
            vard = plotset['all_variables']
            plotvar = vard[varid]
            ...
            for aux in varopts:
                ...
                if '5' in snum and package.upper() == 'LMWG' and opts['output']['json'] == True:
                    ...
                    if snum == '14' and package.upper() == 'AMWG': #Taylor diagrams
                        ...
                        plot = sclass( modelfts, obsfts, varids, time, region, vvaropts[aux],
                                       plotparms = { 'model':{}, 'obs':{}, 'diff':{} } )
                    else:
                        plot = sclass(
                            modelfts, obsfts, varid, time, region, vvaropts[aux], names=names,
...
def general_plot( rsr, plot, title1, title2, vcanvas, vcanvas2, tm, tm2, var, varIndex, displayunits, tp,
                          file_descr, source_descr2):
    ...
    var = var_save
    return tp, source_descr2

def taylor_diagram(plot, rsr, vcanvas, tm, var, title):
    """Taylor diagram need to be rewritten."""
...
def vector_plot(rsr, plot, vcanvas, vcanvas2, tm2, var, title, tmobs, ir, file_descr, source_descr2 ):
    strideX = rsr.strideX
    strideY = rsr.strideY
    ratio = "autot"
    try:
...
def scatter_plot(rsr_presentation, ir, rsr, plot, title, vcanvas, tm, vcanvas2, tm2, var, xvar, yvar, varIndex, file_descr, source_descr2):

    if len(rsr.vars) == 1:
        # scatter plot for plot set 12
        subtitle = title
        vcanvas.plot(var, rsr_presentation, tm, bg=1, title=title, source=rsr.source)
        ...
        try:
            if tm2 is not None and varIndex + 1 == len(rsr.vars):
                ...
                vcanvas2.plot(var, rsr_presentation, tm2, bg=1, title=title, source=subtitle)
                if file_descr != 'diff':
                    source_descr2.append(rsr.source)
        except vcs.error.vcsError as e:
            logger.exception("Making summary plot: %s", e)
        ...
        if varIndex == 0:
            ...
            vcanvas.plot(xvar, yvar, rsr_presentation, tm, bg=1, title=title, source=rsr.source)
        ...
        try:
            if tm2 is not None and varIndex + 1 == len(rsr.vars):
                ...
                vcanvas2.plot(xvar, yvar, rsr_presentation, tm2, bg=1, title=title, source=subtitle)
                if file_descr != 'diff':
                    source_descr2.append(rsr.source)
        except vcs.error.vcsError as e:
            logger.exception("Making summary plot: %s", e)
        ...
        for rsr in resr:
            ...
            for varIndex, var in enumerate(rsr.vars):
                ...
                if vcs.isscatter(rsr.presentation) or (plot.number in ['11', '12'] and package.upper() == 'AMWG'):
                    #this must get called for both plot sets 11 and 12
                    if hasattr(plot, 'customizeTemplates'):
                        if hasattr(plot, 'replaceIds'): #only ps 12
                            var = plot.replaceIds(var)
                        tm, tm2 = plot.customizeTemplates( [(vcanvas, tm), (vcanvas2, tm2)],
                                                           data=var, varIndex=varIndex, graphicMethod=rsr_presentation,
                                                           var=var, iteration=ir )
                    if len(rsr.vars) == 2:
                        #only plot set 11 has 2 variables LWCF and SWCF
                        if varIndex == 0:
                            # first pass through just save the array
                            xvar = var.flatten()
                            savePNG = False
                            ...
                            yvar = var.flatten()
                            savePNG = True
                            scatter_plot(rsr_presentation, ir, rsr, plot, title, vcanvas, tm, vcanvas2, tm2, var,
                                         xvar, yvar, varIndex, file_descr, source_descr2)
                    else:
                        #this gets executed for both plotsets 11 and 12.
                        #for 11, the reverse diagonal line gets overplotted
                        xvar = None
                        yvar = None
                        scatter_plot(rsr_presentation, ir, rsr, plot, title, vcanvas, tm, vcanvas2, tm2, var,
                                     xvar, yvar, varIndex, file_descr, source_descr2)
                ...
                ############# vector plot
                    ...
                    vector_plot(rsr, plot, vcanvas, vcanvas2, tm2, var, title, tmobs, ir, file_descr, source_descr2)

                ############# Taylor diagram
                elif vcs.istaylordiagram(rsr.presentation):
                    taylor_diagram(plot, rsr, vcanvas, tm, var, title)

                ############# general plot
                else:
                    tp, source_descr2 = \
                        general_plot(rsr, plot, title1, title2, vcanvas, vcanvas2, tm, tm2, var, varIndex,
                                         displayunits, tp, file_descr, source_descr2)
